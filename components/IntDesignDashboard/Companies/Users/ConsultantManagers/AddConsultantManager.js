...
const initialFormData = {
  ...
  country_code: "",
  state: "",
  user_type: CONSULTANT_MANAGER_USER_TYPE,
  lat: "",
  long: "",
};

const AddConsultantManager = ({ navigation }) => {
  ...
  useEffect(() => {
    const getAdminId = async () => {
      const adminProfile = await AsyncStorage.getItem("profile");
      setFormData({ ...formData, parent_id: JSON.parse(adminProfile).id });
    ...
    if (
      ...
      try {
        ...
        if (
          ...
          Toast.show("New Consultant Manager Added", {
          ...
          Toast.show("Cannot Add New Consultant Manager", {
            duration: Toast.durations.SHORT,
            position: Toast.positions.BOTTOM,
            shadow: true,
            animation: true,
...
export default AddConsultantManager;
